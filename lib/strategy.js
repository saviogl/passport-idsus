...
function IdsusStrategy(options, verify) {
  ...
  if (!options.loginURL) {
    throw new RangeError('IdsusStrategy options requires a loginURL key');
  }
  if (!options.clientID) {
  ...
  if (!options.apiURL) {
    throw new RangeError('IdsusStrategy options requires a apiURL key');
  }
  ...
  this.options.loginURL = this.options.loginURL.charAt(this.options.loginURL.length - 1) === '/' ? this.options.loginURL.substr(0, this.options.loginURL.length - 1) : this.options.loginURL;

  this.options.apiURL = this.options.apiURL.charAt(this.options.apiURL.length - 1) === '/' ? this.options.apiURL.substr(0, this.options.apiURL.length - 1) : this.options.apiURL;
  ...
  this.idsus = IdentSus({
    ...
    login_url: this.options.loginURL,
